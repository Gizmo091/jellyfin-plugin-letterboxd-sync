...
for cmd in "${DOTNET_PATHS[@]}"; do
    if [ -x "$cmd" ] || command -v "$cmd" &> /dev/null; then
        echo "  Trouvé: $cmd"
        if $cmd --list-sdks 2>/dev/null | grep -q "6.0"; then
            echo "    ✅ .NET 6.0 SDK détecté avec ce binaire"
            DOTNET_CMD="$cmd"
            HAS_NET6=true
            break
        else
            echo "    ⚠️  Pas de .NET 6.0 SDK avec ce binaire"
        fi
    fi
done

if [ -z "$DOTNET_CMD" ] || [ "$HAS_NET6" = false ]; then
    echo ""
    echo "⚠️  .NET 6.0 SDK n'a pas été trouvé."
    echo ""
    echo "   Si vous avez installé .NET 6 via les binaires Microsoft:"
    echo "   export PATH=\"/usr/local/share/dotnet:\$PATH\""
    echo ""
    echo "   Pour installer .NET 6.0 sur macOS:"
    echo "   • Via les binaires Microsoft: https://dotnet.microsoft.com/en-us/download/dotnet/6.0"
    ...
    echo "   Éditez LetterboxdSync.Tests.csproj et changez <TargetFramework>net6.0</TargetFramework>"
